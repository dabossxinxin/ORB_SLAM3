...
void Tracking::PreintegrateIMU() {
  if (!mCurrentFrame.mpPrevFrame) {
    Verbose::Print("Tracking", Verbose::WARN, "Have no previous frame!");
  ...
  if (mlQueueImuData.empty()) {
    Verbose::Print("Tracking", Verbose::WARN,
                   "Have no imu data in mlQueueImuData!");
    mCurrentFrame.setIntegrated();
    return;
  }
  ...
  const int nImu = mvImuFromLastFrame.size();
  if (mvImuFromLastFrame.empty()) {
    Verbose::Print("Tracking", Verbose::WARN, "Empty IMU measurements vector");
    return;
  }
  ...
  for (int i = 0; i < nImu; ++i) {
    ...
    if (!mpImuPreintegratedFromLastKF) {
      Verbose::Print("Tracking", Verbose::WARN,
                     "mpImuPreintegratedFromLastKF does not exist!");
    }
  ...
  mCurrentFrame.setIntegrated();
  Verbose::Print("Tracking", Verbose::INFO,
                 "Preintegration finished, integrated delta t: %.3f",
                 mCurrentFrame.mpImuPreintegratedFrame->dT);
}
...
bool Tracking::PredictStateIMU() {
  ...
  if (mbMapUpdated && mpLastKeyFrame) {
    ...
    Verbose::Print(
        "Tracking", Verbose::WARN,
        "Map not updated, and we have no last keyframe to predict state");
  }
...
void Tracking::Track() {
  if (bStepByStep) {
    Verbose::Print("Tracking", Verbose::INFO, "Waiting to the next step");
  ...
  if (mpLocalMapper->mbBadImu) {
    Verbose::Print("Tracking", Verbose::ERROR,
  ...
  if (!pCurrentMap) {
    Verbose::Print("Tracking", Verbose::ERROR,
                   "There is not an active map in the atlas");
    return;
  }

  // recognize if there is a big jump in timestamps or rollback in time.
  if (mState != NO_IMAGES_YET) {
    if (mLastFrame.mTimeStamp > mCurrentFrame.mTimeStamp) {
      Verbose::Print(
          "Tracking", Verbose::ERROR,
      ...
      if (mpAtlas->isInertial()) {
        if (mpAtlas->isImuInitialized()) {
          Verbose::Print("Tracking", Verbose::WARN,
                         "Timestamp jump detected. State set to LOST. Reseting "
                         "IMU integration...");
  ...
  // Preintegrate IMU measurements from last frame to the current frame
  if ((mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO ||
       mSensor == System::IMU_RGBD) &&
      !mbCreatedMap) {
#ifdef REGISTER_TIMES
  ...
  if (mState == NOT_INITIALIZED) {
    ...
    if (mState != OK)  // If rightly initialized, mState=OK
    {
      ...
      Verbose::Print("Tracking", Verbose::INFO, "Not initialized yet!");
      return;
    }
    ...
    if (!mbOnlyTracking) {
      ...
      if (mState == OK) {
        ...
        if ((!mbVelocity && !pCurrentMap->isImuInitialized()) ||
          ...
          Verbose::Print("Tracking", Verbose::INFO,
                         "Track reference keyframe, status: %d", bOK);
        } else {
          bOK = TrackWithMotionModel();
          Verbose::Print("Tracking", Verbose::INFO,
                         "Track with motion model, status: %d", bOK);
          if (!bOK) {
            bOK = TrackReferenceKeyFrame();
            Verbose::Print("Tracking", Verbose::INFO,
                           "Track reference keyframe, status: %d", bOK);
        ...
        if (mState == RECENTLY_LOST) {
          bOK = true;
          // use imu in recently lost state
          if ((mSensor == System::IMU_MONOCULAR ||
            ...
            if (mCurrentFrame.mTimeStamp - mTimeStampLost > mTimeRecentlyLost) {
              bOK = false;
              mState = LOST;
              Verbose::Print("Tracking", Verbose::WARN,
                             "Predict with imu so long time, set lost...");
            }
            Verbose::Print("Tracking", Verbose::WARN,
                           "Recently lost, predict with imu, status: %d", bOK);
          } else {  // we only have visual sensor, only can use relocalization
            bOK = Relocalization();
            if (mCurrentFrame.mTimeStamp - mTimeStampLost > 3.0f && !bOK) {
              bOK = false;
              mState = LOST;
              Verbose::Print("Tracking", Verbose::WARN, "Track Lost...");
            }
            Verbose::Print("Tracking", Verbose::WARN,
                           "Recently lost, relocalization, status: %d", bOK);
          }
        } else if (mState == LOST) {
          if (pCurrentMap->KeyFramesInMap() < 10) {
            mpSystem->ResetActiveMap();
            Verbose::Print("Tracking", Verbose::WARN, "Reset active map...");
          } else {
            CreateMapInAtlas();
            Verbose::Print("Tracking", Verbose::WARN,
                           "Tracking lost, Create new map in atlas...");
          }
      ...
      if (mState == LOST) {
        bOK = Relocalization();
        Verbose::Print("Tracking", Verbose::WARN, "Relocalization, status: %d",
                       bOK);
      } else {
        if (!mbVO) {
    ...
    // set current frame's reference keyframe
    ...
    // we have an initial estimation. track the local map.
    if (!mbOnlyTracking) {
      if (bOK) {
        bOK = TrackLocalMap();
        Verbose::Print("Tracking", Verbose::INFO, "Track local map, status: %d",
                       bOK);
      } else {
        Verbose::Print("Tracking", Verbose::WARN,
                       "Coarse track failed, do not track local map!");
      }
    } else {
    ...
    if (pCurrentMap->isImuInitialized()) {
      if (bOK) {
        if (mCurrentFrame.mnId == (mnLastRelocFrameId + mnFramesToResetIMU)) {
          ResetFrameIMU();
          Verbose::Print("Tracking", Verbose::INFO,
                         "Resetting frame after relocalization...");
    ...
    if (bOK || mState == RECENTLY_LOST) {
      ...
      for (auto lit = mlpTemporalPoints.begin(); lit != mlpTemporalPoints.end();
           ++lit) {
        MapPoint* pMP = *lit;
        delete pMP;
      }
    ...
    if (mState == LOST) {
      if (pCurrentMap->KeyFramesInMap() <= 10) {
        mpSystem->ResetActiveMap();
        Verbose::Print("Tracking", Verbose::INFO,
                       "Track lost and map too small, reseting...");
        return;
      }
      if (mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO ||
          mSensor == System::IMU_RGBD)
        if (!pCurrentMap->isImuInitialized()) {
          mpSystem->ResetActiveMap();
          Verbose::Print("Tracking", Verbose::INFO,
                         "Track lost before IMU initialization, reseting...");
          return;
        }

      CreateMapInAtlas();
      Verbose::Print("Tracking", Verbose::INFO,
                     "Track lost, create new map in atlas...");
      return;
    }
  ...
  if (mState == OK || mState == RECENTLY_LOST) {
    if (mCurrentFrame.isSet()) {
      Sophus::SE3f Tcr = mCurrentFrame.GetPose() *
...
void Tracking::StereoInitialization() {
  if (mCurrentFrame.N > 500) {
    ...
    Verbose::Print("Tracking", Verbose::INFO,
                   "Stereo Initialization Successful. New Map created with " +
    ...
    mpAtlas->GetCurrentMap()->mvpKeyFrameOrigins.emplace_back(pKFini);
    mpMapDrawer->SetCurrentCameraPose(mCurrentFrame.GetPose());
    mState = OK;
  }
}
...
bool Tracking::TrackWithMotionModel() {
  ...
  if (nmatches < 20) {
    Verbose::Print("Tracking", Verbose::INFO, "Not enough matches!!");
    if (mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO ||
        mSensor == System::IMU_RGBD)
      return true;
    else
...
bool Tracking::TrackLocalMap() {
  ...
  if (!mpAtlas->isImuInitialized()) {
    Optimizer::PoseOptimization(&mCurrentFrame);
    Verbose::Print("Tracking", Verbose::INFO,
                   "Track local map only use visual info");
  } else {
    if (mCurrentFrame.mnId <= mnLastRelocFrameId + mnFramesToResetIMU) {
      Optimizer::PoseOptimization(&mCurrentFrame);
      Verbose::Print("Tracking", Verbose::INFO,
      ...
      // if we have updated map, optimize pose with last keyframe, else with
      // last frame
      if (!mbMapUpdated) {
        inliers = Optimizer::PoseInertialOptimizationLastFrame(&mCurrentFrame);
        Verbose::Print("Tracking", Verbose::INFO,
                       "Track local map with last frame, inliers: %d", inliers);
      } else {
        inliers =
            Optimizer::PoseInertialOptimizationLastKeyFrame(&mCurrentFrame);
        Verbose::Print("Tracking", Verbose::INFO,
                       "Track local map with last keyframe, inliers: %d",
                       inliers);
      }
    }
  }
  ...
  Verbose::Print("Tracking", Verbose::INFO, "Track local map match inliers: %d",
                 mnMatchesInliers);
...
bool Tracking::NeedNewKeyFrame() {
  // if IMU is not initialized, insert keyframe every 0.25 seconds
  if ((mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO ||
       mSensor == System::IMU_RGBD) &&
  ...
  // In only tracking mode, do not insert keyframes
  if (mbOnlyTracking) {
    return false;
  }
  ...
  // Do not insert keyframes if we have enough keyframes from last reloc
  ...
  // potentially created. In Stereo or RGB-D
  ...
  if (mSensor != System::MONOCULAR && mSensor != System::IMU_MONOCULAR) {
    ...
    // Verbose::Print("Tracking", Verbose::INFO,
    //                "[NEEDNEWKF]-> closed points: " + to_string(nTrackedClose)
    //                +
    //                    "; non tracked: " + to_string(nNonTrackedClose));
    // Verbose::INFO);
  }
...
void Tracking::CreateNewKeyFrame() {
  ...
  if (mpLastKeyFrame) {
    ...
    Verbose::Print("Tracking", Verbose::WARN, "No last KF in KF creation!!");
  }
...
void Tracking::SearchLocalPoints() {
  ...
  for (auto vit = mCurrentFrame.mvpMapPoints.begin();
       vit != mCurrentFrame.mvpMapPoints.end(); ++vit) {
    MapPoint* pMP = *vit;
    if (pMP) {
      if (pMP->isBad()) {
  ...
  for (auto vit = mvpLocalMapPoints.begin(); vit != mvpLocalMapPoints.end();
    ...
    // Project (this fills MapPoint variables for matching), degree 60
  ...
  if (nToMatch > 0) {
    ...
    Verbose::Print("Tracking", Verbose::INFO,
                   "Search %d local points in TrackLocalMap", matches);
  }
}
...
void Tracking::UpdateLocalKeyFrames() {
  ...
  std::map<KeyFrame*, int>::const_iterator it = keyframeCounter.begin();
  for (; it != keyframeCounter.end(); ++it) {
    ...
    if (it->second > covisibleMax) {
      covisibleMax = it->second;
      pKFmax = pKF;
    }
  ...
  const size_t localKeyframesLimitNum = 80;
  std::vector<KeyFrame*>::const_iterator itKF = mvpLocalKeyFrames.begin();
  for (; itKF != mvpLocalKeyFrames.end(); ++itKF) {
    // Limit the number of keyframes
    if (mvpLocalKeyFrames.size() > localKeyframesLimitNum) {
      break;
    }
  ...
  if ((mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO ||
       mSensor == System::IMU_RGBD) &&
      mvpLocalKeyFrames.size() < localKeyframesLimitNum) {
  ...
  if (pKFmax) {
    ...
    Verbose::Print(
        ...
        "Current reference keyframe set to %ld, covisible map point num: %d",
        mpReferenceKF->mnId, covisibleMax);
  }
}

bool Tracking::Relocalization() {
  Verbose::Print("Tracking", Verbose::INFO, "Starting relocalization");
  // Compute Bag of Words Vector
  mCurrentFrame.ComputeBoW();
  ...
  if (vpCandidateKFs.empty()) {
    Verbose::Print("Tracking", Verbose::INFO, "There are not candidates");
    return false;
  }
...
void Tracking::Reset(bool bLocMap) {
  Verbose::Print("Tracking", Verbose::INFO, "System Reseting!");
...
void Tracking::ResetActiveMap(bool bLocMap) {
  Verbose::Print("Tracking", Verbose::INFO, "Active map Reseting");
  if (mpViewer) {
    mpViewer->RequestStop();
    while (!mpViewer->isStopped()) {
      usleep(3000);
  ...
  Verbose::Print("Tracking", Verbose::INFO,
                 "mnFirstFrameId: %d, mnInitialFrameId: %d", mnFirstFrameId,
                 mnInitialFrameId);
  ...
  Verbose::Print("Tracking", Verbose::INFO, "%d frames set to lost", num_lost);
...
void Tracking::UpdateFrameIMU(const float s, const IMU::Bias& b,
  ...
  auto lend = mlRelativeFramePoses.end();
  for (; lit != lend; lit++, lRit++, lbL++) {
    if (*lbL) {
      continue;
    }
  ...
  if (mLastFrame.mnId == mLastFrame.mpLastKeyFrame->mnFrameId) {
    ...
    float t12 = mLastFrame.mpImuPreintegrated->dT;  // from last keyframe
