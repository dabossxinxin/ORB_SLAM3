...
void EdgeSE3ProjectXYZOnlyPose::linearizeOplus() {
  if (_vertices[0] == nullptr) {
    printf(
        "EdgeSE3ProjectXYZOnlyPose::linearizeOplus: null vertex[0] edge=%p\n",
        (void*)this);
    return;
  }
  ...
  if (!std::isfinite(xyz_trans[0]) || !std::isfinite(xyz_trans[1]) ||
      !std::isfinite(xyz_trans[2])) {
    printf(
        "EdgeSE3ProjectXYZOnlyPose::linearizeOplus: non-finite xyz_trans "
        "(%f,%f,%f) edge=%p\n",
        xyz_trans[0], xyz_trans[1], xyz_trans[2], (void*)this);
...
void EdgeSE3ProjectXYZOnlyPoseToBody::linearizeOplus() {
  if (_vertices[0] == nullptr) {
    printf(
        "EdgeSE3ProjectXYZOnlyPoseToBody::linearizeOplus: null vertex[0] "
        "edge=%p\n",
        (void*)this);
    return;
  }
  ...
  if (!std::isfinite(X_l[0]) || !std::isfinite(X_l[1]) ||
      !std::isfinite(X_l[2]) || !std::isfinite(X_r[0]) ||
      !std::isfinite(X_r[1]) || !std::isfinite(X_r[2])) {
    printf(
        "EdgeSE3ProjectXYZOnlyPoseToBody::linearizeOplus: non-finite X_l or "
        "X_r edge=%p\n",
        (void*)this);
    _jacobianOplusXi.setZero();
    return;
...
void EdgeSE3ProjectXYZ::linearizeOplus() {
  if (_vertices[1] == nullptr || _vertices[0] == nullptr) {
    printf("EdgeSE3ProjectXYZ::linearizeOplus: null vertex edge=%p\n",
           (void*)this);
    return;
  }
  ...
  if (!std::isfinite(xyz_trans[0]) || !std::isfinite(xyz_trans[1]) ||
      !std::isfinite(xyz_trans[2])) {
    printf(
        "EdgeSE3ProjectXYZ::linearizeOplus: non-finite xyz_trans (%f,%f,%f) "
        "edge=%p\n",
        xyz_trans[0], xyz_trans[1], xyz_trans[2], (void*)this);
    _jacobianOplusXi.setZero();
...
void EdgeSE3ProjectXYZToBody::linearizeOplus() {
  if (_vertices[1] == nullptr || _vertices[0] == nullptr) {
    printf("EdgeSE3ProjectXYZToBody::linearizeOplus: null vertex edge=%p\n",
           (void*)this);
    return;
  }
  g2o::VertexSE3Expmap* vj = static_cast<g2o::VertexSE3Expmap*>(_vertices[1]);
  ...
  if (!std::isfinite(X_l[0]) || !std::isfinite(X_l[1]) ||
      ...
      !std::isfinite(X_r[1]) || !std::isfinite(X_r[2])) {
    printf(
        "EdgeSE3ProjectXYZToBody::linearizeOplus: non-finite X_l or X_r "
        "edge=%p\n",
        (void*)this);
    _jacobianOplusXi.setZero();
    _jacobianOplusXj.setZero();
