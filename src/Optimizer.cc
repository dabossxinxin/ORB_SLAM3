...
void Optimizer::BundleAdjustment(const std::vector<KeyFrame*>& vpKFs,
  ...
  Verbose::Print("Optimizer", Verbose::INFO, "BA: End of the optimization");
...
void Optimizer::FullInertialBA(Map* pMap, int its, const bool bFixLocal,
  ...
  for (size_t i = 0; i < vpKFs.size(); ++i) {
    ...
    if (pKFi->mPrevKF && pKFi->mnId <= maxKFid) {
      ...
      if (pKFi->bImu && pKFi->mPrevKF->bImu) {
        ...
        Verbose::Print("Optimizer", Verbose::WARN,
                       "KF %zu or KF %zu have no imu measures!", pKFi->mnId,
                       pKFi->mPrevKF->mnId);
    }
  }

  // Add prior edge of bias
  if (bInit) {
  ...
  // Add map points vertex and visual edge
...
void Optimizer::OptimizeEssentialGraph(
  ...
  set<std::pair<long unsigned int, long unsigned int>> sInsertedEdges;
...
void Optimizer::OptimizeEssentialGraph(
  ...
  Verbose::Print("Optimizer", Verbose::INFO,
                 "Opt_Essential: vpFixedKFs loaded");
...
void Optimizer::LocalInertialBA(KeyFrame* pKF, bool* pbStopFlag, Map* pMap,
  ...
  const size_t maxCovKF = 0;
  for (size_t i = 0, iend = vpNeighsKFs.size(); i < iend; i++) {
...
void Optimizer::InertialOptimization(Map* pMap, Eigen::Matrix3d& Rwg,
                                     ...
                                     float priorA) {
  Verbose::Print("Optimizer", Verbose::INFO, "inertial optimization");
  int its = 200;
  long unsigned int maxKFid = pMap->GetMaxKFid();
  const std::vector<KeyFrame*> vpKFs = pMap->GetAllKeyFrames();
  ...
  for (size_t i = 0; i < vpKFs.size(); ++i) {
    ...
    if (pKFi->mPrevKF && pKFi->mnId <= maxKFid) {
      ...
      if (!pKFi->mpImuPreintegrated) {
        Verbose::Print("Optimizer", Verbose::ERROR,
      ...
      if (!VP1 || !VV1 || !VG || !VA || !VP2 || !VV2 || !VGDir || !VS) {
        ...
        ss_msg << "Vertex Error: " << VP1 << ", " << VV1 << ", " << VG << ", "
               << VA << ", " << VP2 << ", " << VV2 << ", " << VGDir << ", "
               << VS;
        Verbose::Print("Optimizer", Verbose::ERROR, ss_msg.str());
        continue;
      }
      ...
      vppUsedKF.emplace_back(std::make_pair(pKFi->mPrevKF, pKFi));
      optimizer.addEdge(ei);
    }
  }
...
void Optimizer::InertialOptimization(Map* pMap, Eigen::Matrix3d& Rwg,
  ...
  Verbose::Print(
      "Optimizer", Verbose::INFO,
      "Inertial optimization finished. Initial error: %.6f, Final error: %.6f",
      err_ini, err_end);
...
void Optimizer::MergeInertialBA(KeyFrame* pCurrKF, KeyFrame* pMergeKF,
  ...
  for (int i = 0; i < N; i++) {
    ...
    if (pKFi->bImu && pKFi->mPrevKF->bImu && pKFi->mpImuPreintegrated) {
      ...
      Verbose::Print("Optimizer", Verbose::INFO,
                     "ERROR building inertial edge");
  }

  Verbose::Print("Optimizer", Verbose::INFO, "end inserting inertial edges");
