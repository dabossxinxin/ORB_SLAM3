      ...
      if (!CheckNewKeyFrames() && !stopRequested()) {
        if (mpAtlas->KeyFramesInMap() > 2) {
          if (mbInertial && mpCurrentKeyFrame->GetMap()->isImuInitialized()) {
            ...
            Verbose::Print(
                "LocalMapping", Verbose::INFO,
                "CurrentKF ts: %.3f, PrevKF ts: %.3f, Dist: %.3f, Tinit: %.3f",
                mpCurrentKeyFrame->mTimeStamp,
                mpCurrentKeyFrame->mPrevKF->mTimeStamp, dist, mTinit);

            if (!mpCurrentKeyFrame->GetMap()->GetInertialBA2()) {
              if ((mTinit < 10.f) && (dist < 0.02)) {
                Verbose::Print("LocalMapping", Verbose::WARN,
                               "Not enough motion for initializing. "
                               "Reseting... Dist: %.3f, Tinit: %.3f",
                               dist, mTinit);
        ...
        if ((mTinit < 50.0f) && mbInertial) {
          // Enter here everytime local-mapping is caller
...
void LocalMapping::RequestStop() {
  ...
  std::unique_lock<mutex> lock2(mMutexNewKFs);
  mbAbortBA = true;
}
...
void LocalMapping::SetFinish() {
  std::unique_lock<mutex> lock(mMutexFinish);
  mbFinished = true;
  std::unique_lock<mutex> lock2(mMutexStop);
  mbStopped = true;
}
...
void LocalMapping::InitializeIMU(float priorG, float priorA, bool bFIBA) {
  ...
  if (!mpCurrentKeyFrame->GetMap()->isImuInitialized()) {
    ...
    auto itKF = vpKF.begin();
    for (; itKF != vpKF.end(); ++itKF) {
      if (!(*itKF)->mpImuPreintegrated || !(*itKF)->mPrevKF) {
        continue;
      }

      // set initialize velocity and gracity direction
  ...
  // Optimize inertial gravity direction, bias and scale
  ...
  double onlyInertialBACost =
      std::chrono::duration_cast<std::chrono::duration<double>>(t1 - t0)
          .count() *
      0.001;
  Verbose::Print("LocalMapping", Verbose::INFO,
                 "Inertial Only Bundle Adjustment finished, time cost: %.3fs",
                 onlyInertialBACost);
  Eigen::Vector3f gI =
      mRwg.cast<float>() * Eigen::Vector3f(0.0f, 0.0f, -IMU::GRAVITY_VALUE);
  Verbose::Print("LocalMapping", Verbose::INFO,
                 "Optimizated gravity: %.3f %.3f %.3f, scale: %.3f", gI[0],
                 gI[1], gI[2], mScale);
  ...
  // Optimize position, rotation, velocity, bias and point
  ...
  double fullBACost =
      std::chrono::duration_cast<std::chrono::seconds>(t5 - t4).count() * 0.001;
  Verbose::Print("LocalMapping", Verbose::INFO,
                 "Global Bundle Adjustment finished, time cost: %.3fs, "
                 "Updating map ...",
                 fullBACost);
  ...
  while (!lpKFtoCheck.empty()) {
    ...
    if (pKF->bImu) {
      ...
      Verbose::Print("LocalMapping", Verbose::WARN,
                     "KeyFrame %zu not set to inertial!", pKF->mnId);
    }
  ...
  // Correct MapPoints
  auto vpMPs = mpAtlas->GetCurrentMap()->GetAllMapPoints();
  ...
  Verbose::Print("LocalMapping", Verbose::INFO, "Map Update Finished!");
