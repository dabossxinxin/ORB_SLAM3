...
int main(int argc, char** argv) {
  ...
  // Load all sequences: images, timestamps and IMU measurements
  ...
  for (int seq = 0; seq < num_seq; ++seq) {
    ...
    if ((nImages[seq] <= 0) || (nImu[seq] <= 0)) {
      std::cerr << "ERROR: Failed to load images or IMU for sequence" << seq
                << std::endl;
      return 1;
    }
  ...
  for (int seq = 0; seq < num_seq; ++seq) {
    // Seq loop
    int num_rect = 0, proccIm = 0;
    double t_rect = 0.f, t_resize = 0.f, t_track = 0.f;
